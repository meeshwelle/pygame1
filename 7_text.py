...
while running:
    ...
    #Insert Timer
    #경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 #(ms) 경과시간을 100으로 나눠서 초단위로 표시

    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255)) #render은 글자를 그림
    # 출력할 글자 (시간), True, 글자 색상
    screen.blit(timer, (10, 10)) #10, 10은 화면 왼쪽 위에서 조금 오른쪽, 조금 아래
